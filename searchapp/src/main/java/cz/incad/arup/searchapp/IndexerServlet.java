    ...
    CREATE_THUMB {
      ...
      void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        try {
          ...
          File f = new File(imagesDir + filename);

          String ext = filename.substring(filename.lastIndexOf(".") + 1).toLowerCase();
          String outputFile = "";
          if ("pdf".equals(ext)) {
            ImageSupport.mediumPdf(f, req.getParameter("nazev"));
            outputFile = ImageSupport.thumbnailPdfPage(f, 0, req.getParameter("nazev"));
          } else {
            outputFile = ImageSupport.thumbnailImg(f, req.getParameter("nazev"));
          }

          json.put("created file", outputFile);
          json.put("created images", ImageSupport.generated);
        } catch (Exception ex) {
          json.put("error", ex.toString());
    ...
    CREATE_THUMBS_FROMDIR {
      ...
      void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        try {
          ...
          ImageSupport.thumbs(imagesDir + dir);

          json.put("created", imagesDir + dir);
        } catch (Exception ex) {
          json.put("error", ex.toString());
    ...
    CREATE_THUMBS {
      ...
      void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        try {
          ...
          JSONObject r = indexer.createThumbs(Boolean.parseBoolean(req.getParameter("overwrite")));
    ...
    GENERATE_PDF_MEDIUM {
      ...
      void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        try {
          ...
          if (id != null) {
            File f = new File(id);
            ArrayList<String> t = ImageSupport.mediumPdf(f, f.getName());
            jo.put("total pages", t.size());
            jo.put("files", new JSONArray(t));
          } else {

            String dir = req.getParameter("dir");
            Options opts = Options.getInstance();
            String imagesDir = opts.getString("imagesDir");
            File folder = new File(imagesDir + dir);
            for (final File fileEntry : folder.listFiles()) {
              if (fileEntry.isDirectory()) {
                //listFilesForFolder(fileEntry);
              } else {
                ArrayList<String> t = ImageSupport.mediumPdf(fileEntry, fileEntry.getParent());
                jo.put(fileEntry.getName(), t.size());
              }
            }
          }
    ...
    GENERATE_PDFS {
      ...
      void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        try {
          ...
          ImageSupport.pdfs(imagesDir + dir);
